import 'package:test/test.dart';
import 'package:callchimp_dart/djangoflow_openapi.dart';

// tests for SupervisorListResponse
void main() {
  final SupervisorListResponse? instance = /* SupervisorListResponse(...) */ null;
  // TODO add properties to the entity

  group(SupervisorListResponse, () {
    // int count
    test('to test the property `count`', () async {
      // TODO
    });

    // int currentPage
    test('to test the property `currentPage`', () async {
      // TODO
    });

    // List<SupervisorResponse> results
    test('to test the property `results`', () async {
      // TODO
    });

    // int totalPages
    test('to test the property `totalPages`', () async {
      // TODO
    });

  });
}
